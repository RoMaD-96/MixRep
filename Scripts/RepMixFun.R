...
bf_theta_mix <- function(tr, sr, to, so, x = 1, y = 1, null = null,
                         priorsd = priorsd, w = NA) {
  ...
  if (!is.na(alpha)) {
    ...
    alt_H_1 <- normConst(
      tr = tr, to = to, sr = sr, so = so, null = null,
      priorsd = priorsd, x = x, y = y
    )
  }
...
bf_omega_mix <- function(tr, sr, to, so, x = x, y = y, null = null,
                         priorsd = priorsd, w_null = NA, w_alt = NA){
  ...
  if (!is.na(w_null) && !is.na(w_alt)) {
    ...
    null_H_0 <- dnorm(x = tr, mean = null, sd = sqrt(sr^2 + priorsd^2))
    ...
    null_H_0 <- normConst(tr = tr, to = to, sr = sr, so = so, null = null,
                          priorsd = priorsd, x = 1, y = y)
  } else if (x > 1 && y == 1) {
    ## marginal density under H1: omega = 0
    alt_H_1 <- dnorm(x = tr, mean = null, sd = sqrt(sr^2 + priorsd^2))
    
    ## marginal density under H0
    ## integrating likelihood over alpha|H1 ~ Beta(x, 1) prior
    null_H_0 <- normConst(tr = tr, to = to, sr = sr, so = so, null = null,
                          priorsd = priorsd, x = x, y = 1)
  } else {
    alt_H_1 <- NULL
    null_H_0 <- NULL
  }
